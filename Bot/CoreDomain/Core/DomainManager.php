...
namespace Bot\CoreDomain\Core;

use FilesystemIterator;
use Illuminate\Support\Facades\File;
use Illuminate\Support\Str;
use Bot\CoreDomain\AbstractDomain;
use Bot\CoreDomain\Exceptions\DomainExtendException;
use Bot\CoreDomain\Exceptions\DomainNotExistsException;
use Bot\CoreDomain\Infrastructure\Traits\Singleton;
...
class DomainManager
{
    ...
    public function loadDomain($namespace, string $path)
    {
        ...
        if (!is_subclass_of($class, AbstractDomain::class)) {
            throw new DomainExtendException(sprintf('The domain must extend the abstract class %s.', Domain::class));
        }

        if (!is_object($class)) {
            /** @var AbstractDomain $class */
            $class = new $class();
        }
